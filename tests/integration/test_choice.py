def test_read_choices_by_question_id(integration_quiz_api):
...
def test_update_choice_standalone(integration_quiz_api):
    payload = integration_quiz_api.create_question_payload(
        question_text="2 + 2 = ?",
        choices=[
    ...
    update_choice_response = integration_quiz_api.update_choice(
...
def test_delete_choice_standalone(integration_quiz_api):
    payload = integration_quiz_api.create_question_payload()
    create_question_response = integration_quiz_api.create_question(payload=payload)
    assert create_question_response.status_code == 201
    ...
    delete_choice_response = integration_quiz_api.delete_choice(
    ...
    read_question_response = integration_quiz_api.read_choices(question_id=question_id)
